#!/usr/bin/env python3
"""Pipeline: Generate Flow Maps for all 46 problems with v4 prompt

v4 prompt: "STEP은 풀이의 흐름(전략/접근)이 바뀌는 지점마다 구분하되, 의미 없는 세분화는 피하고 필요한 만큼만 나누세요."
"""
...
def main():
    """Run v4 pipeline on all 46 problems"""

    # Configuration
    csv_file = "2024_math_odd.csv"
    output_dir = os.path.join(os.path.dirname(__file__), "outputs", "v4_all")
    ...
    log("=" * 70)
    log("v4 PIPELINE - All 46 Problems")
    log("=" * 70)

    # Load all problems
    log("Loading problems from CSV...")
    problems = load_all_problems(csv_file)
    log(f"Loaded {len(problems)} problems from {csv_file}")
    ...
    for idx, problem in enumerate(problems):
        ...
        log("")
        log("=" * 70)
        log(f"[{idx+1}/{len(problems)}] {prob_id} ({problem['prob_area']}, {problem['prob_point']}점)")
        log("=" * 70)

        # Generate solutions
        log("Generating solutions from LLMs...")
        ...
        log(f"Saved steps to {step_json_path}")

        # Generate flow map
        log("Generating Flow Map...")
        ...
        try:
            ...
            log(f"Flow Map: {len(flow_map.groups)} groups, {len(flow_map.flows)} flows")
            log(f"Saved to {flowmap_path}")
            ...
            log(f"Flow Map generation FAILED: {e}")
            results_summary.append({
                "prob_id": prob_id,
                "prob_area": problem["prob_area"],
                "prob_point": problem["prob_point"],
        ...
        # Save summary after each problem (incremental)
    ...
    log(f"Processed {len(problems)} problems")
    log(f"Success: {sum(1 for r in results_summary if r['success'])}/{len(problems)}")
