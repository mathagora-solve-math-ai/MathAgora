#!/usr/bin/env python3
"""Visualize v4 Flow Maps with Sub-row Fix + Answer Checker

v4 prompt: "의미 없는 세분화는 피하고 필요한 만큼만 나누세요"
...
def visualize_flowmap(flowmap, title="Flow Map", figsize=(14, 10), save_path=None, prob_info=None):
    ...
    for group_idx, group in enumerate(flowmap['groups']):
        ...
        for step in group['steps']:
            ...
            # Text (with step number if multiple, or answer correctness)
            if is_final_answer and ground_truth:
...
def main():
    """Main function"""
    if len(sys.argv) < 2:
        print("Usage: python3 visualize_v4_fixed.py <prob_id>")
        print("\nExample:")
        print("  python3 visualize_v4_fixed.py 2024_odd_common_1")
        sys.exit(1)

    prob_id = sys.argv[1]

    print(f"Loading Flow Map (v4): {prob_id}")
