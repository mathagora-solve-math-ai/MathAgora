    ...
    *,
    ...
    def _convert_one(q: dict) -> str:
        if q.get("postcorrection_text") or q.get("text_source") == "postcorrection":
            return str(q.get("postcorrection_text") or q.get("merged_text") or "").strip()
        qid = str(q.get("qid", ""))
...
class Handler(BaseHTTPRequestHandler):
        ...
        try:
            ...
            if document_type not in ("csat", "sat"):
                try:
                    ...
                    document_type = "csat"
            # Prefer offline DLA-v2 style experiment output for known demo pages.
    ...
    def _handle_solve_stream(self) -> None:
        ...
        document_type = body.get("documentType") or None
    ...
    def log_message(self, fmt: str, *args) -> None:
        ...
        geo = _geo_lookup(ip)
        LOGGER.info("ACCESS ip=%s geo=%s - %s", ip, geo, fmt % args)
