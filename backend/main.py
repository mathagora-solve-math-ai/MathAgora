...
# Thread pool for parallel OCR text conversion (one thread per question)
...
# Fixed number of problems per page for 2026 data; TSV row order = global problem 1, 2, ...
_2026_PROBLEMS_PER_PAGE = 4
# Placeholder bboxes for 2026 (4-problem grid per page)
...
def _ensure_2026_page_json(page_id: str) -> None:
    """2026 demo: if JSON for page_id does not exist, generate it from TSV using merged_text."""
...
def _load_2026_page_texts(page_id: str) -> dict[str, str] | None:
    """2026 demo: load qid -> merged_text from outputs_parsing/2026_math_odd/page_XXX/page_XXX.json.
    Returns None if not found. If JSON is missing, generates it from TSV via _ensure_2026_page_json first."""
    if not page_id or not page_id.startswith("2026_math_odd_page_"):
        return None
...
def _detections_from_sat_stored_json(page_id: str) -> list[dict] | None:
    """SAT sample: if page_id matches sat_math_odd_page_XXX and the stored JSON exists, return detections without running the parser.
    Detection id matches frontend buildProbId format: sat_{page_part}_{qid} (e.g. sat_8_014_26).
    """
    if not page_id or not page_id.startswith("sat_math_odd_page_"):
        return None
    page_part = page_id.replace("sat_math_odd_page_", "", 1).strip()
    if not page_part:
        return None
    folder = f"page_{page_part}" if not page_part.startswith("page_") else page_part
    json_path = WORKSPACE / "data" / "outputs_parsing" / "sat_math_odd" / folder / f"{folder}.json"
    if not json_path.is_file():
        logger.warning("Detect: SAT stored JSON not found (using parser): path=%s", json_path)
        return None
    try:
        data = json.loads(json_path.read_text(encoding="utf-8"))
        questions = data.get("questions") or []
...
def _detections_from_stored_json(page_id: str) -> list[dict] | None:
    """If page_id matches 2026_math_odd_page_XXX or sat_math_odd_page_XXX and a stored page JSON exists,
    return detections from JSON without running the parser. Returns None otherwise (caller runs the parser).
    """
    if not page_id:
        return None
    if page_id.startswith("sat_math_odd_page_"):
        return _detections_from_sat_stored_json(page_id)
    if not page_id.startswith("2026_math_odd_page_"):
        return None
    _ensure_2026_page_json(page_id)
    parts = page_id.split("_page_", 1)
    if len(parts) != 2:
...
def _questions_to_detections(
    ...
    *,
    ...
    2026: if page_id starts with 2026_math_odd_page_XXX, use merged_text from the stored page JSON (reflects text edits made in demo).
    """
...
async def problems_detect(
    ...
    try:
        ...
        # 3) Save page image, crops, OCR results, and LLM-ready TSV to /workspace/data/demo_parsing
        saved_demo = False
        ...
        # 4) If demo was saved, return ids in page_2_1 format so Solve can load from demo_parsing TSV.
        #    If not saved (e.g. plain upload), use a per-request run_id to isolate the converter cache.
