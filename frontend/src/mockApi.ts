  ...
  return dataUrlPromise.then((dataUrl) => {
    return new Promise<string>((resolve, reject) => {
      ...
      img.onload = () => {
        ...
        // Backend stored JSON (SAT/2026) returns normalized bbox (0–1); parser returns pixels. Support both.
        const isNormalized =
          x >= 0 && x <= 1 && y >= 0 && y <= 1 && w > 0 && w <= 1 && h > 0 && h <= 1;
        const px = isNormalized ? x * tw : x;
        const py = isNormalized ? y * th : y;
        const pw = isNormalized ? w * tw : w;
        const ph = isNormalized ? h * th : h;
        const x1 = Math.max(0, Math.min(px, tw - 1));
        const y1 = Math.max(0, Math.min(py, th - 1));
        const w1 = Math.max(1, Math.min(pw, tw - x1));
        const h1 = Math.max(1, Math.min(ph, th - y1));
