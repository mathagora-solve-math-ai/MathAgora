...
export default function App() {
  ...
  const loadDatasetSelection = async (
    ...
    setSolveStates({}); // 2026 포함 모든 샘플 전환 시 이전 페이지 Solve/aggregation 제거
    setActiveTabId(null);
    setStage("upload");
    setError(null);
  };
  ...
  useEffect(() => {
    ...
    setDetectStageIdx(0);
    const timers = [
      setTimeout(() => setDetectStageIdx(1), 13000),
    ];
  ...
  useEffect(() => {
    ...
    Object.keys(solveStates).forEach((problemId) => {
      ...
      fetch(`${base}/api/flowmap/generate`, {
        ...
        .then(async (res) => {
          if (!res.ok) throw new Error((await res.text()) || `Step-Network failed: ${res.status}`);
        ...
        .catch((e) => {
          ...
          setError(e instanceof Error ? e.message : "Step-Network generation failed.");
        });
    });
  ...
  // Scroll to Step-Network when generating starts or when it is ready for the active tab
  useEffect(() => {
    if (!activeTabId) return;
    const ps = solveStates[activeTabId];
    const shouldScroll = ps?.isGeneratingFlowmap || !!ps?.flowmapData;
    if (!shouldScroll || scrolledForFlowmapRef.current.has(activeTabId)) return;
  ...
  const handleGenerateFlowmap = async (problemId: string) => {
    ...
    try {
      ...
      if (!res.ok) {
        throw new Error((await res.text()) || `Step-Network generation failed: ${res.status}`);
      }
  ...
  return (
    <div className="min-h-screen bg-[radial-gradient(circle_at_top,_#fff5da_0%,_#f7f4ef_35%,_#efe7dc_100%)]">
      <div className="mx-auto flex w-full max-w-[1920px] flex-col gap-10 px-6 py-10">
        <header className="flex items-center justify-between">
          ...
          >
            <img
              src={`${(import.meta.env?.BASE_URL ?? "/").replace(/\/$/, "")}/logos/icon.png`}
              alt="MathAgora"
        ...
        {stage === "upload" ? (
          <section className="rounded-3xl border border-amber-100 bg-white/80 p-6 shadow-soft backdrop-blur">
            ...
            {isDetecting ? (
              <div ref={detectProgressRef} className="mt-5 flex flex-col gap-2.5 rounded-2xl border border-amber-100 bg-white p-6 shadow-sm">
                {/* Header */}
          ...
          <section className="rounded-3xl border border-amber-100 bg-white/80 p-6 shadow-soft backdrop-blur">
            <div className="flex flex-col gap-6">
              ...
              <div className="flex flex-wrap items-start justify-between gap-3">
                <div>
                  <h2 className="text-xl font-semibold text-slate-900">Step-Level Multi-Solution and Alignment</h2>
                  <p className="text-sm text-slate-600">
                    Step-by-step solutions from each model. When all models finish, the Step-Network is generated automatically.
                  </p>
                </div>
              ...
              {activePs && activeTabId ? (
                <>
                  ...
                  {(activePs.flowmapData || activePs.isGeneratingFlowmap) ? (
                      ...
                      {activePs.flowmapData ? (
                        <>
                          <div className="flex flex-wrap items-center gap-3">
                            ...
                            >
                              {activePs.isGeneratingFlowmap ? "Regenerating..." : "Regenerate Step-Network"}
                            </button>
                          </div>
                          <h3 className="text-lg font-semibold text-slate-900">Step-Network</h3>
                          <p className="text-sm text-slate-600">
                        ...
                        <div className="flex items-center gap-4 rounded-2xl border border-slate-200 bg-slate-50 px-6 py-5">
                          <span className="h-7 w-7 shrink-0 animate-spin rounded-full border-[3px] border-slate-300 border-t-slate-600" />
                          <div>
                            <p className="text-base font-semibold text-slate-800">Generating Step-Network…</p>
