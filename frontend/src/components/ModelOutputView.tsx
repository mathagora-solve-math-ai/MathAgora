      ...
      {(() => {
        ...
        return (
          <div
            ...
            style={{
              gridTemplateColumns: `repeat(${Math.max(models.length, 1)}, minmax(320px, 1fr))`,
              alignItems: "stretch",
          ...
          >
            {parsedByModel.map(({ model, state, parsed }) => {
              ...
              return (
                <div
                  key={model.modelId}
                  className="flex min-h-full min-w-0 flex-col rounded-2xl border border-amber-100 bg-white p-4 shadow-soft"
                  style={{ minHeight: totalColumnMinH - 32 }}
                >
                  {/* Header: model name can truncate; status + Stop stay visible */}
                  <div className="mb-3 flex flex-shrink-0 items-start justify-between gap-3">
                    <div className="flex min-w-0 flex-1 items-center gap-2">
                      ...
                      <div className="min-w-0 flex-1">
                        <div className="truncate text-base font-semibold text-slate-900" title={model.displayName}>
                          {model.displayName}
                        </div>
                        <div className="truncate text-xs text-slate-400" title={model.version}>
                          {model.version}
                        </div>
                      </div>
                    </div>
